...
async function upsertReconcile(entity, rows, conflictKey) {
  const table = TABLE_NAME[entity];
  const dbRows = (rows || []).map(o => toRowFull(entity, o));
  ...
  const [upsertRes, existingRes] = await Promise.all([
    rest(table + '?on_conflict=' + conflictKey, {
      method: 'POST',
      headers: { Prefer: 'resolution=merge-duplicates,return=minimal' },
      body: JSON.stringify(dbRows),
    }),
    rest(table + '?select=' + conflictKey),
  ]);
  if (!upsertRes.ok) throw new Error(entity + ': ' + (await upsertRes.text().catch(() => '')));
  if (!existingRes.ok) throw new Error(entity + ' (read for prune): ' + (await existingRes.text().catch(() => '')));

  const existingIds = (await existingRes.json()).map(r => r[conflictKey]);
...
async function upsertAccountsReconcile(rows) {
  ...
  const [putResults, existing] = await Promise.all([Promise.all(puts), rest(table + '?select=user')]);
  for (const r of putResults) {
    if (!r.ok) throw new Error('accounts: ' + (await r.text().catch(() => '')));
  }
  if (!existing.ok) throw new Error('accounts (read for prune): ' + (await existing.text().catch(() => '')));

  const existingUsers = (await existing.json()).map(r => r.user);
...
async function saveWholeDb(db) {
  await Promise.all([
    upsertReconcile('places', db.places, 'id'),
    upsertReconcile('teachers', db.teachers, 'id'),
    upsertReconcile('rewards', db.rewards, 'id'),
    saveSettings(db),
  ]);
  await upsertReconcile('classes', db.classes, 'id');
